...
class FewShotVOCDefaultDataset(FewShotVOCDataset):
    ...
    # 仔细看完这段代码之后我认为这个字典最后会有15个键值对(VOC)
    # 这是键的名字 'SPLIT{split}_{shot}SHOT'
    # 其中每个键的值是一个装有字典的列表
    # 通过这行代码判断一下 for class_name in VOC_SPLIT[f'ALL_CLASSES_SPLIT{split}'
    # 字典的元素会有20个字典字典是这样的
    # dict(
    #             type='ann_file',
    #             ann_file=f'data/few_shot_ann/voc/benchmark_{shot}shot/'+f'box_{shot}shot_{class_name}_train.txt',
    #             ann_classes=[class_name])
    # 再仔细思考一下,对于for shot in [1, 2, 3, 5, 10]来说确实取到的图片是不一样的
    # 但是对于for split in [1, 2, 3]来说，由于shot确定文件，所以说对于同一个shot下的不论是哪个split
    # 下的键对应的列表中的字典元素会是一致的，唯一的不同点在于这些字典的排列顺序会不同
    voc_benchmark = {
    ...
    # FewShotVOCDefaultDataset类获得注释的方法
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        # 调用FewShotVOCDataset父类也就是BaseFewShotDataset类的ann_cfg_parser方法获得注释
