...
class MogaNet(BaseModule):
    ...
    def __init__(self,
        ...
        cur_block_idx = 0
        for i, depth in enumerate(self.depths[:stages]):
            # 颈部倒是非常简单,就是若干个卷积块
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None and self.pretrained is None:
    ...
    def forward(self, x,use_meta_conv: bool = False):
        if use_meta_conv:
            x=self.meta_conv(x)
...
# 重写moganet的共享头部分
# 共享头只接受一种通道大小的特征图
class MetaRCNNMogaLayer(BaseModule):
    ...
    def __init__(self,      
        ...
        self.stage=stage
        self.arch_settings = self.arch_zoo[arch]
        for arch_re in self.model_urls.keys():
            if arch in arch_re:
                pretrained1=self.model_urls[arch_re]
        self.init_cfg = dict(type='Pretrained', checkpoint=pretrained1)
        ...
        # 创建单阶段的颈部层
        patch_embed = ConvPatchEmbed(
        in_channels=self.embed_dims[stage-1],
        embed_dims=self.embed_dims[stage],
        kernel_size=patch_sizes[stage],
        stride=patch_sizes[stage] // 2 + 1,
        norm_type=conv_norm_type)

        # 获得该阶段的模块个数
        depth=self.depths[stage]
        cur_block_idx = sum(self.depths[:stage])

        blocks = nn.ModuleList([
            MogaBlock(
                embed_dims=self.embed_dims[stage],
                ffn_ratio=self.ffn_ratios[stage],
                drop_rate=drop_rate,
                drop_path_rate=dpr[cur_block_idx + j],
                norm_type=conv_norm_type,
                init_value=init_value,
        ...
        norm = build_norm_layer(stem_norm_type, self.embed_dims[stage])

        # 给模型注册模块
        self.add_module(f'patch_embed{stage + 1}', patch_embed)
        self.add_module(f'blocks{stage + 1}', blocks)
        self.add_module(f'norm{stage + 1}', norm)

        # 最大池化，卷积核大小为2，步长默认与卷积核大小相同
        self.max_pool = nn.MaxPool2d(2)
        self.sigmoid = nn.Sigmoid()
        self.con1_subtract=nn.Sequential(
            nn.Conv2d(1024,320,1),
            nn.BatchNorm2d(320),
            nn.ReLU(inplace=True))
        self.con2_raise=nn.Sequential(
    ...
    def forward_features(self, x):
        ...
        x=self.con1_subtract(x)
        patch_embed = getattr(self, f'patch_embed{self.stage + 1}')
        blocks = getattr(self, f'blocks{self.stage + 1}')
        norm = getattr(self, f'norm{self.stage + 1}')
